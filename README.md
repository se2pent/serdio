...
- 🚫 **防重复推荐**：最近播过的歌（含 Live/Remix 换皮）自动进入禁播清单，优先探索喜爱歌手的其他作品
- 🎤 **歌词舞台**：播放时当前句高亮居中跟随，聊天卡片同步显示歌词进度
- 🔊 **DJ 语音重播**：每条 DJ 消息带 `talking` 按钮，可重播语音；老消息点击时现场合成，一次合成永久有效
- ⏰ **节律调度（可关）**：晨间唤醒 / 午间 / 专注 / 夜色 / 深夜五档作息，整点情绪检查；`.env` 设 `SCHEDULER_ENABLED=false` 关闭，DJ 只在你召唤时工作
- 🌤️ **天气情绪换装**：界面主题色随真实天气变化（晴 / 阴 / 雨 / 雪 / 雾 / 雷 七种情绪色）
- 🎵 **VIP 完整播放**：扫码登录网易云后，VIP 歌曲完整时长（扫码入口在设置页内）
- 🧹 **轻交互**：一键清屏（DJ 记忆保留）、回到顶部悬浮钮、迷你播放器吸顶
...
| 聊天气泡里的曲目单 | ▶ 点卡片切歌，♡ 喜欢，⏭ 跳过（DJ 会记住你的反馈）；正在播的卡片实时显示歌词进度 |
| DJ 消息 `talking` 钮 | 重播那段 DJ 语音；老消息点击时现场合成，一次合成永久有效 |
| 大播放器 / 迷你条 | 翻阅聊天时大控件自动收起为吸顶迷你条；滚回顶部自动展开 |
| ↑ 回顶钮 / 🧹 清屏 | 右下角悬浮钮一键回顶；清屏只收起聊天界面，DJ 的记忆保留 |
| ♡ 档案页 | 查看口味画像 / 日常作息 / 反馈记录 |
| ⚙ 设置页 | API Key 管理（脱敏显示）/ DJ 音色 / 天气城市 / 网易云扫码登录 / 服务状态 |

**打断规则**：聊天与切歌会正常打断当前播放；调度器推送则在当前歌放完后无缝接入新队列，不打扰正在听的歌（调度器可整体关闭，见下）。
...
安全组放行 `22` 和 `8080`，然后 `.env` 里加两行并 `pm2 restart serdio`：

```bash
API_TOKEN=你的访问密码        # 公网上锁：所有接口验令牌，App 首次打开输一次密码
SCHEDULER_ENABLED=false      # 关闭定时推送（晨间唤醒/整点情绪检查），DJ 只在你召唤时工作，省 API 额度
```
